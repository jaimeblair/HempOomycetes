...
## Alpha Diversity plots as mean + sd (not boxplots)
```{r}
#+++++++++++++++++++++++++
...
COI_ACE <- data_summary(COIstats, varname = "ACE", groupnames = "Location")
COI_Shan <- data_summary(COIstats, varname = "Shannon", groupnames = "Location")
p1 <- ggplot(COI_ACE, aes(x=Location, y=ACE, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=ACE-sd, ymax=ACE+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange", "orange", "orange")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1), 
  ...
  ggtitle("ACE")
p2 <- ggplot(COI_Shan, aes(x=Location, y=Shannon, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=Shannon-sd, ymax=Shannon+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange", "orange", "orange")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1), 
        legend.position = "none", 
...
RPSforACE <- FullRPSstats[-c(4,8,9,12,13), ]
RPS_ACE <- data_summary(RPSforACE, varname = "ACE", groupnames = "Location")
RPS_Shan <- data_summary(RPSstats, varname = "Shannon", groupnames = "Location")
p3 <- ggplot(RPS_ACE, aes(x=Location, y=ACE, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=ACE-sd, ymax=ACE+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1), 
        legend.position = "none", 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
  ...
  ylab("Alpha Diversity Measure") +
  ggtitle("ACE")
p4 <- ggplot(RPS_Shan, aes(x=Location, y=Shannon, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=Shannon-sd, ymax=Shannon+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange")) +
...
ITS_ACE <- data_summary(ITSstats, varname = "ACE", groupnames = "Location")
ITS_Shan <- data_summary(ITSstats, varname = "Shannon", groupnames = "Location")
p5 <- ggplot(ITS_ACE, aes(x=Location, y=ACE, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=ACE-sd, ymax=ACE+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange", "orange", "orange")) +
  ...
  ylab("Alpha Diversity Measure") +
  ggtitle("ACE")
p6 <- ggplot(ITS_Shan, aes(x=Location, y=Shannon, color=Location)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=Shannon-sd, ymax=Shannon+sd), width=0.2, size=0.9) +
  theme_bw() +
  scale_color_manual(values = c("darkgreen", "darkgreen", "darkgreen", "orange", "orange", "orange")) +
...
This analysis uses an alternative metadata file with the soil fertility data included.
```{r}
#Use same count_df and taxa_df variables from above

# Read in new metadata file
COImetadataFert <- read.csv("COI_metadataFert.csv")
RPSmetadataFert <- read.csv("rps10_metadataFert.csv")
ITSmetadataFert <- read.csv("ITS_metadataFert.csv")

# Make the sample names the row names for sample information
COImetadataFert <- COImetadataFert %>% remove_rownames %>% column_to_rownames(var="SampleName")
COImetadataFert <- as.data.frame(COImetadataFert)
RPSmetadataFert <- RPSmetadataFert %>% remove_rownames %>% column_to_rownames(var="SampleName")
RPSmetadataFert <- as.data.frame(RPSmetadataFert)
ITSmetadataFert <- ITSmetadataFert %>% remove_rownames %>% column_to_rownames(var="SampleName")
ITSmetadataFert <- as.data.frame(ITSmetadataFert)

# Transform data into phyloseq object
COIsample_info_Fert <- sample_data(COImetadataFert)
COIcount_tab_phy <- otu_table(COIcount_df, taxa_are_rows=T) 
COItax_tab_phy <- tax_table(COItaxa_df)

COIPhyloseq_Fert <- phyloseq(COIcount_tab_phy,
                            COItax_tab_phy,
                            COIsample_info_Fert)
COIPhyloseq_Fert #Print phyloseq summary

RPSsample_info_Fert <- sample_data(RPSmetadataFert)
RPScount_tab_phy <- otu_table(RPScount_df, taxa_are_rows=T) 
RPStax_tab_phy <- tax_table(RPStaxa_df)

RPSPhyloseq_Fert <- phyloseq(RPScount_tab_phy,
                            RPStax_tab_phy,
                            RPSsample_info_Fert)
RPSPhyloseq_Fert #Print phyloseq summary

ITSsample_info_Fert <- sample_data(ITSmetadataFert)
ITScount_tab_phy <- otu_table(ITScount_df, taxa_are_rows=T) 
ITStax_tab_phy <- tax_table(ITStaxa_df)

ITSPhyloseq_Fert <- phyloseq(ITScount_tab_phy,
                            ITStax_tab_phy,
                            ITSsample_info_Fert)
ITSPhyloseq_Fert #Print phyloseq summary

#Remove mocks from dataset
COIeDNA_PhyloseqFert <- subset_samples(COIPhyloseq_Fert, HempType != "Mock")
COIeDNA_PhyloseqFert
RPSeDNA_PhyloseqFert <- subset_samples(RPSPhyloseq_Fert, HempType != "Mock")
RPSeDNA_PhyloseqFert
ITSeDNA_PhyloseqFert <- subset_samples(ITSPhyloseq_Fert, HempType != "Mock")
ITSeDNA_PhyloseqFert

#Remove missing data from rps10
RPSnoMissFert <- subset_samples(RPSeDNA_PhyloseqFert, Location != "LocationC")
RPSnoMissFert <- subset_samples(RPSnoMissFert, Location != "LocationE")
RPSnoMissFert <- subset_samples(RPSnoMissFert, SampleID != "D")
RPSnoMissFert

# Transform to even sampling depth (convert counts to percentages)
COIeDNA_transformedFert = transform_sample_counts(COIeDNA_PhyloseqFert, function(x) x/sum(x)*100)
RPSeDNA_transformedFert = transform_sample_counts(RPSnoMissFert, function(x) x/sum(x)*100)
ITSeDNA_transformedFert = transform_sample_counts(ITSeDNA_PhyloseqFert, function(x) x/sum(x)*100)

COI_cca <- ordinate(COIeDNA_transformedFert, method = "CCA", formula = ~ Temp + pH + CN + P + NO3.N + K + Mg + Ca + Cu + Zn + S)
sink(file = "COIfit.txt")
summary(COI_cca)

#plot and calculate envfit in vegan
COImetaFert <- as(sample_data(COIeDNA_transformedFert), "data.frame")
COImetaFert <- select(COImetaFert, Temp, pH, CN, P, NO3.N, K, Mg, Ca, Cu, Zn, S)
#pairs(COImetaFert[,11:15], pch=19)
COIfit <- envfit(COI_cca, COImetaFert, perm=999)
COIfit
sink(file = NULL)
plot(COI_cca, display = NULL)
#plot(COI_cca, display="wa", type = "points")
plot(COIfit, p.max = 0.05) #have to call ordination plot first to avoid plot.new error


RPS_cca <- ordinate(RPSeDNA_transformedFert, method = "CCA", formula = ~ Temp + pH + CN + P + NO3.N + K + Mg + Ca + Cu + Zn + S)
sink(file = "RPSfit.txt")
summary(RPS_cca)

#plot and calculate envfit in vegan
RPSmetaFert <- as(sample_data(RPSeDNA_transformedFert), "data.frame")
RPSmetaFert <- select(RPSmetaFert, Temp, pH, CN, P, NO3.N, K, Mg, Ca, Cu, Zn, S)
RPSfit <- envfit(RPS_cca, RPSmetaFert, perm=999)
RPSfit
sink(file = NULL)
plot(RPS_cca, display = NULL)
#plot(RPS_cca, display="wa", type = "points")
plot(RPSfit, p.max=0.05) #have to call ordination plot first to avoid plot.new error


ITS_cca <- ordinate(ITSeDNA_transformedFert, method = "CCA", formula = ~ Temp + pH + CN + P + NO3.N + K + Mg + Ca + Cu + Zn + S)
sink(file = "ITSfit.txt")
summary(ITS_cca)

#plot and calculate envfit in vegan
ITSmetaFert <- as(sample_data(ITSeDNA_transformedFert), "data.frame")
ITSmetaFert <- select(ITSmetaFert, Temp, pH, CN, P, NO3.N, K, Mg, Ca, Cu, Zn, S)
ITSfit <- envfit(ITS_cca, ITSmetaFert, perm=999)
ITSfit
sink(file = NULL)
plot(ITS_cca, display = NULL)
#plot(ITS_cca, display="wa", type = "points")
plot(ITSfit, p.max=0.05) #have to call ordination plot first to avoid plot.new error
