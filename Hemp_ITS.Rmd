...
For ITS: Forward primer (R1) = 19 bps, Reverse primer (R2) = 19 bps
...
## Filter and Trim
ITS reads have some lower quality at ends so trim to 200. Use minLen to remove abundant short sequences (unknown contamination?).
```{r}
...
out <- filterAndTrim(fnFs, #file path to the directory containing the forward fastq files
                     ...
                     truncLen=c(200,200), #truncate reads after truncLen bases, default is 0 (no truncation) 
...
For ITS: the target length is 200-325
```{r}
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 200:325]
dim(seqtab2)
...
track
write.table(track, "ITS_ReadStats.tsv", sep="\t", quote=F, col.names=NA)
...
write(asv_fasta, "ITS_ASVs.fasta")

asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ITS_ASVs_counts.tsv", sep="\t", quote=F, col.names=NA)
```
...
write.table(fas,file="ITS_OTUs.fasta", row.names=FALSE, col.names=FALSE,quote = FALSE)

otu_tab <- t(merged_seqtab)
write.table(otu_tab, "ITS_OTUs_counts.tsv", sep="\t", quote=F, col.names=NA)
...
ASVtaxa <- assignTaxonomy(seqtab.nochim, #the merged sequences with chimeras removed
                       "UNITEhead_Oomycota.fasta", #reference sequences
                       minBoot=80,
                       multithread=TRUE,
                       tryRC=FALSE)
...
write.table(asv_tax, "ITS_ASVs_taxa.tsv", sep = "\t", quote=F, col.names=NA)

# Merge ASV tables for manual analysis
Counts <- read_tsv("ITS_ASVs_counts.tsv", show_col_types = FALSE)
colnames(Counts)[1]=c('ASV')
Taxa <- read_tsv("ITS_ASVs_taxa.tsv", show_col_types = FALSE)
colnames(Taxa)[1]=c('ASV')
Merged <- inner_join(Counts, Taxa, by="ASV")
write_tsv(Merged, "ITS_DADA2_ASVsmerged.tsv")
...
write.table(otu_tax, "ITS_OTUs_taxa.tsv", sep = "\t", quote=F, col.names=NA)

# Merge OTU tables for manual analysis
Counts2 <- read_tsv("ITS_OTUs_counts.tsv", show_col_types = FALSE)
colnames(Counts2)[1]=c('OTU')
Taxa2 <- read_tsv("ITS_OTUs_taxa.tsv", show_col_types = FALSE)
colnames(Taxa2)[1]=c('OTU')
Merged2 <- inner_join(Counts2, Taxa2, by="OTU")
write_tsv(Merged2, "ITS_DADA2_OTUsmerged.tsv")
...
ASVspecies <- assignSpecies(seqtab.nochim,
                       "UNITEhead_OomSpecies.fasta",
                       allowMultiple = TRUE,
                       tryRC=FALSE)
# Create ASV taxonomy table for species analysis
asv_sp <- ASVspecies
rownames(asv_sp) <- gsub(pattern=">", replacement="", x=asv_headers)
write.table(asv_sp, "ITS_ASVs_species.tsv", sep = "\t", quote=F, col.names=NA)

# Merge ASV tables for manual analysis
SpeciesList <- read_tsv("ITS_ASVs_species.tsv", show_col_types = FALSE)
colnames(SpeciesList)[1]=c('ASV')
SpMerged <- inner_join(Merged, SpeciesList, by="ASV")
write_tsv(SpMerged, "ITS_DADA2_ASVsSpeciesmerged.tsv")
...
write.table(otu_sp, "ITS_OTUs_species.tsv", sep = "\t", quote=F, col.names=NA)

# Merge OTU tables for manual analysis
OTUSpeciesList <- read_tsv("ITS_OTUs_species.tsv", show_col_types = FALSE)
colnames(OTUSpeciesList)[1]=c('OTU')
SpMerged2 <- inner_join(Merged2, OTUSpeciesList, by="OTU")
write_tsv(SpMerged2, "ITS_DADA2_OTUsSpeciesmerged.tsv")
